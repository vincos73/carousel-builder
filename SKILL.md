...
12. Se è selezionato `local-editor`, creare il manifest in stato `bozza` e completare il preflight visuale descritto in [references/visual-review.md](references/visual-review.md) prima di consegnare l'editor: nessuna slide iniziale deve mostrare avvisi di densità o overflow in uno dei tre sistemi proposti. Correggere o dividere il copy finché il fit reale è pulito, senza ridurre il carattere oltre l'8%. Poi avviare l'editor e aprirlo nel browser. Mostrare nell'editor le varianti di logo realmente disponibili, una sintesi dei caratteri usati e i comandi contestuali per grassetto, corsivo, sottolineatura, evidenziatore e commento. Non limitarsi a stampare il codice HTML nella chat e non duplicare l'intera anteprima, salvo richiesta dell'utente o fallback.
...
1. Dopo `Approva la prova visuale`, produrre l'intera sequenza con un renderer o adapter compatibile quando disponibile. Un produttore è compatibile soltanto se dichiara e implementa il `visual_style_system` selezionato; palette e font corretti non compensano l'assenza della sua firma strutturale.
