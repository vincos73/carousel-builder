...
1. Saltare questa fase se il manifest è già a `prova_visuale_approvata`. Se `cover_mode` richiede un visuale, tradurre 2-3 concetti in una metafora secondo [cover-visual.md](references/cover-visual.md), poi generare o acquisire l'immagine. Nel locale collegarla con `attach_cover_asset.py`, mai modificando il manifest a mano. Se la modalità è `typographic` o l'immagine non è producibile, mantenere una copertina tipografica completa.
2. Applicare la grammatica del sistema visivo selezionato alle card interne, alla chiusura e alla copertina tipografica. Una copertina `generated` o `provided` resta invece nella griglia split con immagine. Convertire le enfasi approvate in campi espliciti e rimuovere gli asterischi.
