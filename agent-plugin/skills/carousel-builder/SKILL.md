...
5. Avviare l'editor secondo [visual-review.md](references/visual-review.md), con `--return-thread-id` quando disponibile. Riutilizzare una sola tab e chiamare `markHandoff()` come documentato. Restare in attesa attiva con intervalli massimi di 50 secondi: non terminare il turno né chiedere «fatto». Il batch resta durevole. All'evento `feedback`, confermare subito ID, azione e prossimo passo, poi eseguire `process_review.py`.
6. Elaborare il batch con `process_review.py <manifest> <feedback> --session-dir <session-dir>`. Per `feedback`, applicare commenti e correzioni dell'utente. Per `approve`, copy, ordine, profilo ed enfasi del batch sono definitivi: non confrontarli con fonte o bozze, correggerli, ripristinarli o riaprire il checkpoint per valutazioni editoriali, tipografiche o di accessibilità. Dopo l'approvazione bloccano soltanto errori tecnici, strutturali, di sicurezza o del workflow.
...
In caso di errore non avanzare lo stato e preservare gli artefatti validi. Nel percorso locale seguire [review-recovery.md](references/review-recovery.md) soltanto dopo un'interruzione o conflitto. Dopo un timeout dell'agente, riavviare il server con lo stesso manifest e la stessa sessione, applicare l'eventuale batch durevole e riprendere senza chiedere una nuova approvazione. Un errore di soli metadati tecnici può rielaborare lo stesso batch senza nuovo consenso. Le correzioni riaprono atomicamente `bozza` se cambiano copy, ordine o profilo, oppure `testi_approvati` se cambiano solo stile, logo o enfasi. Ripetere solo i checkpoint invalidati e non modificare stato o ricevute a mano.
