...
Nel percorso `local-editor`, `advance_workflow.py` non si limita a estensione, presenza e digest: decodifica struttura, CRC, scanline e filtri di ogni PNG, verifica le dimensioni effettive e controlla la contact sheet alla geometria prevista. Per il PDF segue `startxref`, tabella xref, `trailer.Root`, Catalog e albero Pages; richiede una pagina per slide e `MediaBox` 810×1012,5 pt, equivalente al raster 1440×1800 px usato dall'export. File troncati, strutture non collegate al Root, dimensioni errate e input oltre i limiti dichiarati bloccano `rendering -> qa` anche se il digest attestato coincide.
