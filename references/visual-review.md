...
5. Mantenere il processo in ascolto in ogni checkpoint dell'editor e non concludere dopo l'apertura. Confermare subito ogni batch. Il server applica `approve`; `feedback` attende l'agente. Dopo la conferma HTTP l'editor mostra il batch registrato e continua il polling. In Codex Desktop usa un `return_url` canonico per mostrare `Torna alla chat`. Altrove compare solo con `--return-thread-id`. Riusare la tab, chiamare `markHandoff()` a inizio lavoro e prima di fermarsi e reclamare binding stale. Il ritorno avviene solo su click.
   Attendere sul processo fino a 50 secondi per volta, riusando il suo identificativo, finché arriva un evento o l'utente interrompe. Non inviare una risposta finale durante l'attesa.
6. Senza output, leggere `session-state.json`. Se `last_feedback_id` differisce da `applied_feedback_id`, usare `last_action` e `last_feedback_path`. Se coincide, attendere anche `processed_feedback_id` e lo stato `processed`, oppure gestire `approval_blocked` o `error`. Solo per stati legacy usare `feedback.json`.
...
`/api/session` espone sotto lock `render_fingerprint` e feedback durevole. Il fingerprint lega snapshot, produzione, bundle e asset. Su `approve`, il browser invia fingerprint e `base_workflow_state`; il server deriva lo stage. `/api/status` espone checkpoint, `applied_feedback_id`, `processed_feedback_id`, stato ed errore di elaborazione. Sono il solo contratto dell'esito: niente alias legacy.
...
Per un batch `feedback`, leggere `warnings`, `stale_alt_text` e `stale_transcript` e applicare le richieste dell'utente; spazi e ritorni a capo sono normalizzati e non producono avvisi di accessibilità. Per un batch `approve`, copy, ordine, profilo ed enfasi inviati dall'editor sono autoritativi: gli avvisi consultivi non autorizzano una seconda revisione dell'agente, la rigenerazione del copy o la riapertura del checkpoint. Proseguire direttamente quando `process_review.py` raggiunge `prova_visuale_approvata`; fermarsi soltanto su `approval_blocked` o su un errore tecnico, strutturale, di sicurezza o del workflow. Un `approve` visuale non include modifiche editoriali; un feedback vuoto non riapre checkpoint.
