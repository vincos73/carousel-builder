...
5. Mantenere il processo in ascolto in ogni checkpoint dell'editor. Non concludere il turno dopo l'apertura né chiedere «fatto». Confermare subito in chat ogni batch ricevuto. Il server applica automaticamente i batch `approve` attraverso `process_review.py`; i batch `feedback` restano in attesa dell’agente perché commenti e note richiedono interpretazione. Dopo la conferma HTTP l’editor deve mostrare uno stato statico di batch registrato e continuare il polling senza rotella. In Codex Desktop il server richiede un `return_url` e mostra `Torna alla chat` dopo l'invio e negli stati successivi, usando solo `codex://threads/<thread-id>`. Altrove compare solo con `--return-thread-id`: non è un requisito dell’editor. A ogni turno acquisire o riusare la tab e chiamare `markHandoff()` prima del lavoro e prima di una possibile fine; reclamare una tab visibile con binding stale. Il ritorno avviene solo dopo il click dell'utente, mai automaticamente.
