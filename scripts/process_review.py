...
from review_core import LockUnavailableError, strict_json_loads  # noqa: E402
...
def _feedback_stage(feedback_path: Path) -> tuple[str, str | None]:
    try:
...
def process_review(
    ...
    *,
    ...
    action, approval_stage = _feedback_stage(feedback_path)
    ...
    target = APPROVAL_TARGETS.get(approval_stage or apply_result.get("approval_stage"))
    if target is None:
        raise ValueError("L'approvazione non dichiara il checkpoint da avanzare")
    if status.get("workflow_state") == target:
    ...
    result["workflow"] = build_status(manifest_path, session_dir_input=session_dir)
